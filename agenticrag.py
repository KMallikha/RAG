...
#vec_emb=Chroma.from_documents(documents=rp,embedding=emb,collection_name="docdata",persist_directory="./chromadb")
db = Chroma(persist_directory="./chromadb", embedding_function=emb,collection_name="docdata")
"""re=db.similarity_search("what is ocean pollution",k=2)
print(re)"""

llm=ChatGroq(api_key=apikey,model="groq/compound-mini")
retriever= db.as_retriever(search_kwargs={"score_threshold":0.7})
...
tools=[Tool(
    name="rag",
    func=chain.run,
    description="use this tool if question is related to the acellor compoff policy"
)]
